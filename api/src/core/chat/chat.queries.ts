...
export type TransactionSummaryGroupBy =
  | "merchant"
  | "budgetItem"
  | "account"
  | "type";
...
const buildBudgetItemSummary = (
  ...
  if (!budget || !category) {
    throw new Error(
      `Budget item ${budgetItem.id} is missing related budget or category`,
    );
  }

  const relatedTransactions =
    transactionsByBudgetItemId.get(budgetItem.id) ?? [];
  const latestTransaction = sortByDate(
    relatedTransactions,
  ...
  return {
    ...
    latestTransactionDate: latestTransaction
      ? toIsoString(latestTransaction.date)
      : null,
...
async function getUserData(userId: string) {
  ...
  const categoryMap = mapById(
    categories.map((category) => createCategorySummary(category)!),
  );
  const accountMap = mapById(
    accounts.map((account) => createAccountSummary(account)!),
  );
  const transactionsByBudgetItemId = new Map<string, typeof transactions>();

  for (const transaction of transactions) {
    const current =
      transactionsByBudgetItemId.get(transaction.budgetItemId) ?? [];
    current.push(transaction);
    transactionsByBudgetItemId.set(transaction.budgetItemId, current);
  }

  const budgetItemSummaries = budgetItems.map((budgetItem) =>
    buildBudgetItemSummary(
      budgetItem,
      budgetMap,
      categoryMap,
      transactionsByBudgetItemId,
    ),
  );
  ...
  const recurringTemplateSummaries = recurringTransactions
    ...
    .filter(
      (template): template is RecurringTransactionSummary => template !== null,
    );
...
const filterTransactions = (
  ...
  return transactions.filter((transaction) => {
    if (
      ...
      transaction.budgetItemId !== filters.budgetItemId
    ) {
      return false;
    }
    ...
    if (
      ...
      (transaction.recurringTemplateId ?? null) === null
    ) {
      return false;
    }
...
export async function listBudgets(
  ...
  return budgets
    .filter((budget) =>
      filters.month === undefined ? true : budget.month === filters.month,
    )
    .filter((budget) =>
      filters.year === undefined ? true : budget.year === filters.year,
    )
    .sort((left, right) => {
...
export async function getBudget(
  ...
  const items = budgetItemSummaries.filter(
  ...
  return budgetItemSummaries
    .filter((item) =>
      filters.budgetId === undefined
        ? true
        : item.budgetId === filters.budgetId,
    )
    .filter((item) =>
    ...
    .sort(
      (left, right) =>
        ensureDate(
          right.updatedAt ?? right.createdAt ?? new Date(0),
        ).getTime() -
...
export async function getBudgetItem(
  ...
  const { budgetItemMap, accountMap, recurringTemplateMap, transactions } =
  ...
  const recentTransactions = filterTransactions(transactions, budgetItemMap, {
    budgetItemId,
  })
    .sort(
      (left, right) =>
        ensureDate(right.date).getTime() - ensureDate(left.date).getTime(),
    )
...
export async function analyzeBudgetItemSpend(
  ...
  const relevantTransactions = transactions.filter(
    (transaction) => transaction.budgetItemId === budgetItemId,
  );
...
export async function listTransactions(
  ...
  const { accountMap, budgetItemMap, recurringTemplateMap, transactions } =
    await getUserData(userId);
...
export async function getTransaction(
  ...
  const { accountMap, budgetItemMap, recurringTemplateMap } =
    await getUserData(userId);
...
export async function summarizeTransactions(
  ...
  filters: Omit<
    TransactionFilters,
    "accountId" | "merchantQuery" | "recurringOnly" | "limit"
  > & {
    groupBy?: TransactionSummaryGroupBy;
  },
  ...
  const relevantTransactions = filterTransactions(
    transactions,
    budgetItemMap,
    filters,
  );
  ...
  const createGroupMetadata = (
    transaction: (typeof relevantTransactions)[number],
  ) => {
    if (groupBy === "merchant") {
      return {
  ...
  return {
    ...
    groups: [...groups.values()].sort(
      (left, right) => right.netTotal - left.netTotal,
    ),
  };
}
...
export async function listRecurringTransactions(
  ...
  return recurringTransactions
    ...
    .map(
      (transaction) =>
        createRecurringTransactionSummary(transaction, categoryMap)!,
    )
    .filter(
      (transaction): transaction is RecurringTransactionSummary =>
        transaction !== null,
    );
}
...
export async function summarizeRecurringCommitments(
  ...
  for (const transaction of recurringTransactions) {
    ...
    const currentRecurringDate = byRecurringDate.get(
...
export async function validateChatScope(
  ...
  if (params.budgetItemId) {
    checks.push(
      getBudgetItemByUserAndId(userId, params.budgetItemId).then(
        (budgetItem) => {
          if (!budgetItem) {
  ...
  if (params.templateId) {
    checks.push(
      getRecurringTransactionByUserAndId(userId, params.templateId).then(
        (template) => {
          if (!template) {
            throw new Error(
              `Recurring transaction ${params.templateId} not found`,
            );
          }
        },
      ),
  ...
  if (params.transactionId) {
    checks.push(
      getTransactionByUserAndId(userId, params.transactionId).then(
        (transaction) => {
          if (!transaction) {
