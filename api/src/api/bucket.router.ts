...
import { InsertBucketSchema, UpdateBucketSchema } from "../core/bucket/bucket.model.js";
import { createBucket, readBucketsFromUser, bucketUpdate, removeBucket } from "../core/bucket/bucket.useCase.js";
...
bucketRouter.post("/", userMiddleware, zValidator("json", InsertBucketSchema.omit({ userId: true })), async (c) => {
  const userId = await getAuthenticatedUserId(c.var.userId);
  if (!userId) {
    return c.json({ error: "Unauthorized" }, 401);
  }
...
bucketRouter.put("/:bucketId", userMiddleware, zValidator("json", UpdateBucketSchema), async (c) => {
  const userId = await getAuthenticatedUserId(c.var.userId);
  if (!userId) {
    return c.json({ error: "Unauthorized" }, 401);
  }
