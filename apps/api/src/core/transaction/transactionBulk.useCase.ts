...
const guessMerchant = (source: Record<string, string>) => {
  return (
    findRowValue(source, ["merchant", "description", "payee", "name", "memo"]) ??
...
const parseCsvRows = async (file: File): Promise<ParsedCsvRow[]> => {
  ...
  if (file.size > MAX_PREVIEW_FILE_BYTES) {
    throw new BulkPreviewTooLargeError("CSV file exceeds the 1 MB preview limit.");
  }
  ...
  if (parsedRows.length === 0) {
    throw new BulkPreviewValidationError("The uploaded CSV does not contain any rows.");
  }
...
const normalizePreviewChunkRows = ({
  ...
  return chunkRows.map((chunkRow) => {
    ...
    const merchant =
      modelRow?.merchant?.trim() || guessMerchant(chunkRow.source) || `Row ${chunkRow.rowIndex}`;
    ...
    const resolvedAccount = resolveAccount(modelRow?.accountName ?? null, accounts);
...
const validateCommittedReferences = async (
  ...
  const accountIds = Array.from(new Set(request.rows.map((row) => row.accountId)));
...
export const commitBulkTransactions = async (
  ...
  const committedRows = request.rows.map((row) => {
    ...
    if (Number.isNaN(parsedDate.getTime())) {
      throw new BulkPreviewValidationError("Bulk commit contains an invalid date.");
    }
  ...
  for (const row of committedRows) {
    ...
    signedImpactByBudgetItemId.set(row.budgetItemId, currentAmount + signedAmount);
  }
  ...
  await db.transaction().execute(async (trx) => {
    ...
    const currentBudgetItems = await trx
      .selectFrom("budgetItems")
      .innerJoin("budgets", "budgets.id", "budgetItems.budgetId")
      .select(["budgetItems.id as id", "budgetItems.actualAmount as actualAmount"])
      .where("budgets.userId", "=", userId)
      .where("budgetItems.id", "in", Array.from(signedImpactByBudgetItemId.keys()))
      .execute();
