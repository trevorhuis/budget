...
describe("budget chat tools", () => {
  test("provide read-only budget domain tools with scoped data and computed summaries", async () => {
    ...
    assert.equal((transaction.recurringTemplate as JsonValue).merchant, "Electric Co");

    const transactionSummary = (await getTool("summarize_transactions").execute?.({
      groupBy: "type",
    })) as JsonValue;
    ...
    const recurringTransactions = (await getTool("list_recurring_transactions").execute?.(
      {},
    )) as JsonValue[];
    assert.equal(recurringTransactions.length, 1);
    assert.equal((recurringTransactions[0]?.category as JsonValue).name, "Utilities");

    const recurringTransaction = (await getTool("get_recurring_transaction").execute?.({
      templateId: "019cf45e-80f5-714a-a121-bb32f8365109",
    })) as JsonValue;
    assert.equal(recurringTransaction.realizedTransactionCount, 1);
    assert.equal(recurringTransaction.latestRealizedTransactionDate, "2026-03-15T12:00:00.000Z");

    const recurringSummary = (await getTool("summarize_recurring_commitments").execute?.(
      {},
    )) as JsonValue;
