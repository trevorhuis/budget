...
const stripUpdateFields = <TItem extends CollectionItem>(item: TItem) => {
  const { id, ...payload } = stripServerFields(item)
  return payload
}
...
export const createCrudHandlers = <TItem extends CollectionItem>(
  ...
  onUpdate: async ({ transaction }: { transaction: { mutations: Array<{ modified: TItem; original: TItem }> } }) => {
    const { modified, original } = transaction.mutations[0]
    await PUT(`${baseUrl}/${original.id}`, stripUpdateFields(modified))
  },
  onInsert: async ({ transaction }: { transaction: { mutations: Array<{ modified: TItem }> } }) => {
    const item = transaction.mutations[0].modified
    await POST(baseUrl, stripInsertFields(item))
  },
  onDelete: async ({ transaction }: { transaction: { mutations: Array<{ modified: TItem }> } }) => {
    const item = transaction.mutations[0].modified
